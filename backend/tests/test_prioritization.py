...
class TestPriorityScoring:
    def test_invoice_file_scores_higher_than_metadata(self) -> None:
        invoice = _make_doc(
            "d1", "Lieferantenbuchungen.txt", DocumentType.TXT, "Betrag EUR 50000"
        )
    ...
    def test_bank_statement_scores_higher_than_schema(self) -> None:
        bank = _make_doc(
            "d1", "Sachkontobuchungen.txt", DocumentType.TXT, "Zahlung Betrag Konto"
        )
        schema = _make_doc(
            "d2", "gdpdu-01-08-2002.dtd", DocumentType.UNKNOWN, "<!DOCTYPE>"
        )
    ...
    def test_financial_content_boosts_score(self) -> None:
        financial = _make_doc(
            ...
            "data.csv",
            DocumentType.CSV,
            "EUR 48000 Zahlung Konto Betrag Rechnung",
        )
        generic = _make_doc(
            "d2", "data.csv", DocumentType.CSV, "hello world nothing here"
        )
    ...
    def test_score_between_0_and_1(self) -> None:
        docs = [
            ...
            _make_doc(
                "d3", "Pruefungsplanung_JET_2025.docx", DocumentType.DOCX, "Prüfung"
            ),
        ]
        for doc in docs:
            score = compute_priority_score(doc)
...
class TestRanking:
    ...
    def test_select_start_documents(self) -> None:
        docs = [
            ...
            _make_doc(
                "b", "Sachkontobuchungen.txt", DocumentType.TXT, "Buchung Betrag EUR"
            ),
            _make_doc(
                "c", "Wareneingangsliste_2025.csv", DocumentType.CSV, "Wareneingang EUR"
            ),
